        ...
        const message = res.status === 201 ? `The ${label} has been Created successfully.` :
            res.status === 200 ? `The ${label} has been Modified successfully.` :
                res.status === 203 ? `The ${label} has been Deleted successfully.` :
                    res.status === 204 ? `The ${label} has been Cancelled.` :
                        res.status === 206 ? `The ${label} has been Confirmed.` :
                            res.status === 207 ? `The ${label} has been Rejected.` : res.message;

        return {
            status: res.status,
            message: message,
            isSuccess: res.status === 201 || res.status === 200 || res.status === 203 || res.status === 204 || res.status === 206 || res.status === 207,
            data: res.data.data
        }
    }
    catch (e) {
