...
const Details = ({ customerName, setCustomerName, customerPhone, setCustomerPhone,customerEmail, setCustomerEmail, notes,setNotes, referral,setReferral }) => {
   const { TextArea } = Input;
    return (
        <div class='flex flex-col font-normal gap-4 w-full' >
            ...
            <TextboxFlexCol label={'Referral Cell # '} input={
                <Input placeholder="Enter referral 10 digit phone number . . (Optional)" size="large" value={referral} onChange={(e) => setReferral(setCellFormat(e.target.value))} />
            } />
            <TextboxFlexCol label={'Additional Notes'} input={
                <TextArea
                    rows={3}
