...
const Calender = ({ orderList, servicesList, userList, companyList, trndate, btn_ViewClick }) => {
        ...
        <table class={`text-left h-fit w-full`}>
            ...
            <tbody>
                {generateSlots.map(item => (
                    <tr key={item.id} class="bg-white border-t text-xs overflow-auto whitespace-nowrap border-gray-100 hover:bg-zinc-50 ">
                        ...
                        {headerItems.map(a => (
                            <td class='p-1'>
                                ...
                                {orderList.filter(b => b.assignedto === a.id && b.slot === item.id).map(c => (
                                    <div key={c.id} class={`flex flex-col gap-2 border-s-4 p-2  ${getBorder(c.status)}`}>
