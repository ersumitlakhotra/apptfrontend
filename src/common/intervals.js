...
const AvailableSlots = ({ inTime, outTime, orderListSlot, slot }) => {
    const slots = generateTimeSlots(inTime, outTime, 30, orderListSlot);
    return slots.filter(item => !item.disabled || item.id === slot)
}
const generateTimeSlots = (startHour, endHour, intervalMinutes, orderListSlot, slot) => {
    const slots = [];
    ...
    while (currentTime < endTime) {
        ...
        let disabled = orderListSlot.some(item => item.slot === `${slotTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}` )

        if (slot === `${slotTime.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`)
            disabled=false;
...
export { Slots, generateTimeSlots, AvailableSlots };